  ...
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(
        `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAYcsgYyQYrwth8WUaMKu_F-KsMXhw7w40`,
        {
          method: 'POST',
...
export const loginUser = (email, password) => {
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(
        `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAYcsgYyQYrwth8WUaMKu_F-KsMXhw7w40`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
